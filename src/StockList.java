...
has a code c of 3, 4, 5 or more capitals letters. The 1st letter of a code is the capital 
letter of the book category. In the bookseller's stocklist each code c is followed by a 
space and by a positive integer n (int n >= 0) which indicates the quantity of books of 
this code in stock.
  ...
  public static String stockSummary(String[] lstOfArt, String[] lstOf1stLetter) {
    ...
    for(int i=0; i<lstOf1stLetter.length; i++) {
      for(int ii=0; ii<lstOfArt.length; ii++) {
        if(lstOfArt[ii].charAt(0) == lstOf1stLetter[i].charAt(0)){
          int numbers = Integer.parseInt(lstOfArt[ii].substring(lstOfArt[ii].indexOf(" ")+1));
          sum[i] = sum[i] + numbers;    
        }
      }  
    }
    for(int i=0; i<lstOf1stLetter.length; i++) {
      ...
      catalogue = catalogue + "(" + lstOf1stLetter[i] + " : " + sum[i] + ")";
